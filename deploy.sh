...
#   2. For horizontal: create Secrets Manager secret for RDS Proxy (see config.env for commands)
#   3. Vertical DB password is auto-generated by the stack (no pre-create needed)
...
check_prerequisites() {
  ...
  if [[ ! -f "${CONFIG_FILE}" ]]; then
    ...
    # Horizontal: check Secrets Manager secret exists (Aurora + RDS Proxy need it)
    if aws secretsmanager describe-secret \
        --secret-id "horizontal-scaling/db-credentials" \
        --region "${REGION}" &>/dev/null 2>&1; then
      echo "  [OK]   Secrets Manager secret horizontal-scaling/db-credentials exists"
    else
...
case "${COMMAND}" in
  ...
  *)
    ...
    echo ""
    echo "Commands:"
    echo "  check        Verify prerequisites (AWS CLI, credentials, config.env, Secrets Manager)"
