...
These recipes deploy into your existing VPC — they do not create one. If your account is brand new, AWS creates a [default VPC](https://docs.aws.amazon.com/vpc/latest/userguide/default-vpc.html) in every region automatically. You can use it to try things out.

For the horizontal recipe you also need **private subnets** (for Aurora and Redis) in at least 2 Availability Zones. The default VPC only has public subnets — you will need to add private subnets with a NAT gateway, or create a new VPC. The [VPC wizard](https://docs.aws.amazon.com/vpc/latest/userguide/create-vpc.html) can do this in a few clicks ("VPC and more" option creates both).

**ACM certificate (horizontal only)**

The ALB requires an HTTPS certificate. Request one for free in [ACM](https://console.aws.amazon.com/acm/home) — AWS Certificate Manager:
...
The CloudFront distribution expects a WAF Web ACL in `us-east-1`. Create one in the [WAF console](https://console.aws.amazon.com/wafv2/homev2) (choose "Amazon CloudFront distributions" as the resource type), then copy the ARN into `config.env` as `WAF_ACL_ARN`.
